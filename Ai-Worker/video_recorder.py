...
class ClipRecorder:
    ...
    def update_frame(self, cam_id, frame):
        ...
        # Store frame alongside its capture timestamp
        self.buffers[cam_id].append((frame, time.time()))

        # Feed frame to ALL active recordings for this camera
        keys_to_save = []
        for key, rec in self.recordings.items():
            if rec['cam_id'] == cam_id:
                rec['frames'].append((frame, time.time()))
                rec['remaining'] -= 1
                if rec['remaining'] <= 0:
    ...
    def trigger(self, cam_id, event_type, timestamp):
        ...
        # current_history now contains (frame, timestamp) tuples
        ...
        self.recordings[recording_key] = {
            'cam_id': cam_id,
            'frames': current_history,  # contains (frame, timestamp) tuples
    ...
    def _save_clip(self, recording_key):
        ...
        frames = rec_data['frames']  # list of (frame, timestamp) tuples
    ...
    def _write_video(self, frames, filepath):
        ...
        try:
            # Filter out entries where the frame itself is None
            frames = [f for f in frames if f[0] is not None]
            ...
            if not frames:
                ...
                frames = [(blank, time.time())]

            # Separate images and timestamps
            images = [f[0] for f in frames]
            timestamps = [f[1] for f in frames]

            # Calculate real FPS from actual capture timestamps
            duration = timestamps[-1] - timestamps[0]
            if duration <= 0 or len(images) < 2:
                actual_fps = float(self.fps)
            else:
                actual_fps = len(images) / duration

            # Clamp to avoid crazy values
            actual_fps = max(7.0, min(actual_fps, 25.0))
            print(f"📊 Dynamic FPS: {actual_fps:.2f} ({len(images)} frames over {duration:.2f}s)")
            ...
            out = cv2.VideoWriter(filepath, fourcc, actual_fps, (w, h))
            ...
            out.release()
            print(f"🎬 VIDEO SAVED: {len(images)} frames → {filepath}")
