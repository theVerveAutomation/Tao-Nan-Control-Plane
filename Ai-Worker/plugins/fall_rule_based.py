...
class RuleBasedFallPlugin:
    ...
    def __init__(
        ...
        # Body ratio = width / height of the keypoint bounding box.

        # Upright person: ratio < ~0.5 (tall and narrow)

        # Fallen/collapsed person: ratio > ~1.0 (wide and flat)
        ...
        # Both must be true to trigger — reduces false positives from bending
 
        # How many consecutive frames the person must look fallen before alert

        self.FALLEN_FRAME_CONFIRM = 3    # ~0.1s at 30fps — fast but not instant
        ...
        self.fallen_frame_count = defaultdict(int)  # consecutive fallen frames per track
    ...
    def _compute_torso_angle(kps_xy: np.ndarray) -> float:
        ...
        # arctan2(dx, dy): 0° when perfectly vertical, grows as person tilts
...
> 1.0  → horizontal / fallen
    ...
    def _is_fallen_posture(self, kps_xy: np.ndarray, cam_id: str, track_id) -> tuple[bool, float, float]:
        ...
        Returns (is_fallen, body_ratio, torso_angle).
 
        A person is considered in a fallen posture if BOTH:

          - body_ratio > FALLEN_RATIO_THRESH  (keypoints span is wider than tall)

          - torso_angle > FALLEN_ANGLE_THRESH (torso is tilted past 45° from vertical)
 
        This rejects:

          - Bending forward to pick something up (ratio stays low, torso angle moderate)

          - Sitting (ratio moderate, angle moderate)

          - Walking / crouching briefly (ratio < 1.0)
        ...
        for cam_id, data in scene_state.items():
            ...
            for track_id, person in tracked.items():
                ...
                if is_fallen:
                    ...
                    # Person is upright or just bending — reset counter
